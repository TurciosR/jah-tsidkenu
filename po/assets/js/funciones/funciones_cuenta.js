...
$(document).ready(function () {
  $(".select").select2();
  var centinela_cob = $("#cent_cuenta_cob").val();
  var url = base_url+"Cuenta/get_data";
  $('#editable2').DataTable({
    "pageLength": 50,
    "serverSide": true,
    "order": [[0, "asc"]],
    "ajax": {
      url: url+"/"+centinela_cob,
      type: 'POST'
    },
    "language": {
...
function guardar_abono()
{
    ...
    url+="&saldo="+saldo+"&abonado="+abonado+"&fecha="+fechadd;
    if (error!=1) {
        $.ajax({
            type: 'POST',
