...
$sql_suc=_fetch_array(_query("SELECT CONCAT('Sucursal ',sucursal.n_sucursal,' ',sucursal.direccion) as origen,traslado.id_sucursal_origen,traslado.fecha,traslado.hora  FROM traslado JOIN sucursal ON traslado.id_sucursal_origen=sucursal.id_sucursal WHERE traslado.id_traslado=$id_traslado"));


$sql_empresa = "SELECT * FROM sucursal WHERE id_sucursal=$_SESSION[id_sucursal]";

$destino=$sql_suc['origen'];
$ho=$sql_suc['hora'];
...
class PDF extends FPDF
{
    ...
    public function Header()
    {
        ...
        $this->Cell(195,6,"REPORTE DE TRASLADO RECIBIDO",0,1,'C');
        $this->Cell(195,6,"TRASLADO: ".str_pad($this->f,7,"0",STR_PAD_LEFT),0,1,'C');
        $this->Cell(195,6,"ORIGEN: ".utf8_decode((Mayu(utf8_decode($this->g)))),0,1,'C');
        ...
        $this->Cell(20, 5, 'ESPERADO', 1, 0, 'L');
        $this->Cell(20, 5, 'RECIBIDO', 1, 1, 'L');
    }
...
$sqlp=_query("SELECT  producto.id_producto, producto.descripcion,presentacion.nombre,presentacion_producto.unidad,traslado_detalle_recibido.cantidad,traslado_detalle_recibido.recibido
FROM traslado_detalle_recibido
JOIN producto ON producto.id_producto=traslado_detalle_recibido.id_producto
JOIN presentacion_producto ON presentacion_producto.id_presentacion=traslado_detalle_recibido.id_presentacion
JOIN presentacion ON presentacion.id_presentacion=presentacion_producto.presentacion WHERE traslado_detalle_recibido.id_traslado=$id_traslado");
...
while ($row=_fetch_array($sqlp)) {
  ...
  $pdf->Cell(20, 5,"".($row['recibido']/$row['unidad']), 1, 1, 'R');
}
$ylinea=$pdf->GetY();
if ($ylinea<255) {
    # code...
