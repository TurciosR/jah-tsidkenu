...
function print_ticket($id_factura){
	...
	if($nrows_fact>0){
		...
		for($i=0;$i<$nrows_fact_det;$i++){
			...
			$info_factura.=$descrip.esp_text($cantidad,12).esp_num($precio_unit,10).esp_num($subtotal,11)."".$e_g."\n";
...
function print_vale($id_movimiento){
  $id_sucursal=$_SESSION['id_sucursal'];
	...
	//consulta
	$sql="SELECT  e.id_empleado, e.nombre,mct.tipo,
	...
	JOIN movimiento_caja_tipo AS mct ON mct.id_tipo=mc.id_tipo
  ...
  $valor= sprintf('%.2f', $valor);
	...
	$info_factura.=$esp_init."TIPO: ".$row['tipo']."\n";
...
function print_corte($id_corte){
	...
	if($id_c<0) {
		...
		FROM controlcaja AS c
		JOIN usuario AS e ON(e.id_usuario=c.id_empleado)
		JOIN empleado as em on(e.id_empleado=em.id_empleado)
		WHERE c.id_corte='$id_corte'";
	}
	...
	if($tipo=="C"){
		...
		$info_factura.=$esp_init0."SALDO CAJA CHICA $:     ".str_pad(number_format($monto_ch,2,".",""),12," ",STR_PAD_LEFT)."\n";
	...
	if($tipo=="X" || $tipo=="Z"){
		...
		$info_factura.=$esp_init0."FACTURAS:   ".esp_text($finicio,7)."  ".esp_text($ffinal,7).$sp3.$totalnof."\n";
		$sp1=len_num($cfinicio,$n);
		$sp2=len_num($cffinal,$n);
		$sp3=len_num($totalnocf,$n);
		$info_factura.=$esp_init0."FISCALES:   ".esp_text($cfinicio,7)."  ".esp_text($cffinal,7).$sp3.$totalnocf."\n";
		$info_factura.=$line1;
...
function print_fact($id_factura,$tipo_id,$nombreapecte,$direccion){
	...
	if($nrows_fact>0){
		...
		$arrayL[6]=  p_set($arrayL[6],$fecha_fact,77,99,"B");


		$arrayL[9]= p_set($arrayL[9],quitar_spc($nombre_ape),15,99,"R");

		$arrayL[11]= p_set($arrayL[11],quitar_spc($direccionx),17,70,"R");

		$arrayL[12]= p_set($arrayL[12],$nodp,18,50,"R");
		...
		for($i=0;$i<$nrows_fact_det;$i++){
			...
			$descripcion =trim($row_fact_det['descripcion']);
			//descripcion presentacion
			$descpre =trim($row_fact_det['preprodesc']);
			$descpresenta =trim($row_fact_det['predesc']);
			$exento=$row_fact_det['exento'];
			$id_factura_detalle =$row_fact_det['id_factura_detalle'];
			$id_prod_serv =$row_fact_det['id_prod_serv'];
			...
			if ($i<22) {
				...
				if ($exento==0){
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,4,10,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),12,60,"R");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$precio_unit,61,71,"L");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$subtotal,86,99,"L");
				}
				else{
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,4,10,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),12,60,"R");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$precio_unit,61,85,"L");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$subtotal,86,99,"L");
				}
	...
	$array_nocon= dtl($total_txt0,44);
	foreach ($array_nocon as $key => $value) {
		// code...
		$arrayL[$array_painc[$key]]=p_set($arrayL[$array_painc[$key]],$value,10,55,"R");
	}

	$arrayL[47]= p_set($arrayL[47],sprintf("%.4f",$SumGravado),86,99,"L");
	$arrayL[49]= p_set($arrayL[49],sprintf("%.4f",$row_fact['retencion']),86,99,"L");
	$arrayL[51]= p_set($arrayL[51],sprintf("%.4f",round(($SumGravado-$row_fact['retencion']),4)),86,99,"L");
	$arrayL[56]= p_set($arrayL[56],sprintf("%.4f",$venta_exenta),86,99,"L");
	$arrayL[58]= p_set($arrayL[58],sprintf("%.2f",round(($SumGravado+$venta_exenta-$row_fact['retencion']),2)),86,99,"L");
