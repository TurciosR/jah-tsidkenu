                ...
                <div class='row' id='form_invent_inicial'>
                  <div class="col-lg-6">
                    <div class="form-group has-info">
                      <label>Cliente</label>
                      <input type='text' class='form-control'  id='cliente_buscar' name='cliente_buscar' value="<?php echo $sql_pedido['id_cliente']." | ".$sql_pedido['nombre'];?>">
                    </div>
                  </div>
                  <div class="col-lg-6">
                    <div class="form-group has-info">
                      <label>Lugar de entrega</label>
                      <input type="text" class='form-control' name="lugar_entrega" id="lugar_entrega" value="<?php echo $sql_pedido['lugar_entrega'];?>">
                    </div>
                  </div>
                </div>
                <div class="row">
                  <div class='col-lg-6'>
                    <div class='form-group has-info'>
                      <label>Fecha</label>
                      <input type='text' class='datepick form-control' value="<?php echo $sql_pedido['fecha'];?>" id='fecha' name='fecha'>
                    </div>
                  </div>
                  <div class='col-lg-6'>
                    <div class='form-group has-info'>
                      <label>Fecha entrega</label>
                      <input type='text' class='datepick form-control' value="<?php echo $sql_pedido['fecha_entrega'];?>" id='fecha2' name='fecha2'>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="ibox-content">
                      ...
                      <section>
                        <table class="table table-striped table-bordered table-condensed">
                          ...
                          <?php
                            ...
                            $cantidad=0;
                             while ( $filas=_fetch_array($sql_p))
                             {
                               ...
                               $sql_reserva=_fetch_array(_query("SELECT SUM((pedido_detalle.cantidad*presentacion_producto.unidad)) AS reservado FROM pedido_detalle JOIN pedido ON pedido.id_pedido=pedido_detalle.id_pedido JOIN presentacion_producto ON presentacion_producto.id_presentacion=pedido_detalle.id_presentacion WHERE pedido_detalle.id_producto='$id_producto' AND pedido.reservado=1 AND pedido.id_sucursal=$_SESSION[id_sucursal]"));
                               $reservado=$sql_reserva['reservado'];
                               $reservado=round($reservado,2);

                               $sql_reserva=_fetch_array(_query("SELECT SUM((pedido_detalle.cantidad*presentacion_producto.unidad)) AS reservado FROM pedido_detalle JOIN pedido ON pedido.id_pedido=pedido_detalle.id_pedido JOIN presentacion_producto ON presentacion_producto.id_presentacion=pedido_detalle.id_presentacion WHERE pedido_detalle.id_producto='$id_producto' AND pedido.reservado=1 AND pedido.id_pedido=$id_pedido AND pedido.id_sucursal=$_SESSION[id_sucursal]"));
                              ...
                              echo "<td><div class='col-xs-1'><input type='hidden' class='unidad' value='".$filas['unidad']."'><input type='text' readonly value='".($filas['stock']-$reservado+$reservado_yo)."' style='width:60px;' class='existencia'></div></td>";
                          ...
                          <tfoot>
                            <tr>
                              <td colspan="6">Total<strong></strong></td>
                      ...
                      <input type="hidden" id="id_pedido" name="id_pedido" value="<?php echo $id_pedido;?>" class="btn btn-primary m-t-n-xs" />
  ...
  function modificar()
  {
    ...
    $where_clause_c="id_pedido='".$id_pedido."'";
    ...
    for ($i=0;$i<$cuantos ;$i++)
    {
      list($id_producto,$precio_compra,$precio_venta,$cantidad,$sutto,$id_presentacion,$id_pedido_detalle)=explode('|',$lista[$i]);
    ...
    if($m)
    {
      ...
      $xdatos['msg']='Pedido actualizado con exito!';
    }
    else
    {
      _rollback();
      $xdatos['typeinfo']='Error';
      $xdatos['msg']='Pedido no pudo ser ingresado!'._error();
    }
  ...
  function eliminar_producto()
  {
    ...
    $table = 'pedido_detalle';
  	//$where_clause = "id_producto='$id_producto' AND id_pedido='$id_pedido'";
    _begin();
    $where_clause = "id_pedido_detalle='".$id_pedido_detalle."'";
  	$delete = _delete ( $table, $where_clause );
  	if ($delete)
  	{
      _commit();
      $xdatos ['typeinfo'] = 'Success';
      $xdatos ['msg'] = 'Producto eliminado del pedido!';
  	}
  	else
  	{ _rollback();
  		$xdatos ['typeinfo'] = 'Error';
  		$xdatos ['msg'] = 'Producto no pudo ser elimimado del pedido!';
  	}
  	echo json_encode ( $xdatos );
  }
