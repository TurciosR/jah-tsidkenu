...
$id_row_p=_fetch_array(_query("SELECT cuenta_pagar.saldo_pend, cuenta_pagar.id_proveedor,
voucher.id_movimiento, voucher.fecha, voucher.numero_doc, voucher.forma_pago,
voucher.monto, voucher.referencia_pago
FROM voucher
JOIN voucher_mov ON voucher_mov.id_movimiento=voucher.id_voucher
JOIN cuenta_pagar ON cuenta_pagar.id_cuenta_pagar=voucher_mov.id_cuenta_pagar
WHERE voucher.id_voucher = $id_voucher LIMIT 1 "));
$id_proveedor=$id_row_p['id_proveedor'];
$id_movimiento=$id_row_p['id_movimiento'];
$fecha2=$id_row_p['fecha'];
$numero_voucher=$id_row_p['numero_doc'];
$forma=$id_row_p['forma_pago'];
$total=$id_row_p['monto'];
$referencia=$id_row_p['referencia_pago'];

$sql_banco=_query("SELECT mov_cta_banco.numero_doc, banco.nombre,cuenta_banco.numero_cuenta FROM mov_cta_banco JOIN banco ON mov_cta_banco.id_cuenta=banco.id_banco JOIN cuenta_banco ON cuenta_banco.id_cuenta=mov_cta_banco.id_cuenta WHERE  mov_cta_banco.id_movimiento=$id_movimiento");
...
$sql_pro=_query("SELECT proveedor.nombreche,proveedor.contacto FROM proveedor WHERE id_proveedor=$id_proveedor");
...
$pdf->setear($banco,$numero_voucher,1);
...
$pdf->Cell(110, 6,''.$propietario, "L", 0, 'L');/*.$this->a*/
if($forma == "Cheque")
{
  $pdf->Cell(50,6,"CUENTA  NO:    ",0,0,'L');
  $pdf->Cell(36,6,''.$numero_de_cuenta,"R",1,'L');
}
else if($forma == "Efectivo")
{
  $pdf->Cell(86,6,'PAGO EN EFECTIVO',"R",1,'L');
}
else
{
  $pdf->Cell(50,6,"TRANSFERENCIA NO:  ",0,0,'L');
  $pdf->Cell(36,6,''.$referencia,"R",1,'L');
}
...
while ($row2=_fetch_array($rs)) {
  $i=1;
  $cargo=0;
  $result=_query("SELECT * FROM detalle_voucher WHERE id_cuenta_pagar=$row2[id_cuenta_pagar] ORDER BY id_detalle ASC");
