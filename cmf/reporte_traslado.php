...
$sql_suc=_fetch_array(_query("SELECT CONCAT('Sucursal ',sucursal.descripcion) as destino,traslado.id_sucursal_origen,traslado.fecha,traslado.hora  FROM traslado JOIN sucursal ON traslado.id_sucursal_destino=sucursal.id_sucursal WHERE traslado.id_traslado=$id_traslado"));
...
$destino=$sql_suc['destino'].", ENVIA: ".$sql_emple['envia'];
$ho=$sql_suc['hora'];
...
class PDF extends FPDF
{
    ...
    public function Header()
    {
        ...
        $this->Cell(35, 5, utf8_decode('PRESENTACIÓN'), 1, 0, 'L');
        $this->Cell(15, 5, 'UNIDAD', 1, 0, 'L');
        $this->Cell(15, 5, 'COSTO', 1, 0, 'L');
        $this->Cell(15, 5, 'CANT', 1, 0, 'L');
        $this->Cell(20, 5, 'SUBTOTAL', 1, 1, 'L');
    }
...
$sqlp=_query("SELECT  producto.id_producto, producto.descripcion,presentacion.nombre,presentacion_producto.unidad,traslado_detalle.cantidad,traslado_detalle.costo
...
while ($row=_fetch_array($sqlp)) {
  ...
  $pdf->Cell(35, 5, utf8_decode($row['nombre']), 1, 0, 'L');
  $pdf->Cell(15, 5, $row['unidad'], 1, 0, 'R');
  $pdf->Cell(15, 5,"".number_format(($row['costo']),2), 1, 0, 'R');
  $pdf->Cell(15, 5,"".($row['cantidad']/$row['unidad']), 1, 0, 'R');
  $pdf->Cell(20, 5,"".number_format((($row['cantidad']/$row['unidad'])*$row['costo']),2), 1, 1, 'R');
  $total=$total+round((($row['cantidad']/$row['unidad'])*$row['costo']),2);
}
$pdf->Cell(175, 5,"TOTAL", 1, 0, 'C');
