...
while(strtotime($fk) <= strtotime($fin1))
{
  ...
  $sql_min_max=_query("
  SELECT MIN(CAST(num_fact_impresa as UNSIGNED)) as minimo, MAX(CAST(num_fact_impresa as UNSIGNED)) as maximo FROM factura WHERE CAST(num_fact_impresa as UNSIGNED) !=0 AND fecha = '$fk' AND numero_doc LIKE '%TIK%' AND id_sucursal = $id_sucursal  UNION ALL
  SELECT MIN(CAST(num_fact_impresa as UNSIGNED)) as minimo, MAX(CAST(num_fact_impresa as UNSIGNED)) as maximo FROM factura WHERE CAST(num_fact_impresa as UNSIGNED) !=0 AND fecha = '$fk' AND numero_doc LIKE '%COF%' AND id_sucursal = $id_sucursal UNION ALL
  SELECT MIN(CAST(num_fact_impresa as UNSIGNED)) as minimo, MAX(CAST(num_fact_impresa as UNSIGNED)) as maximo FROM factura WHERE CAST(num_fact_impresa as UNSIGNED) !=0 AND fecha = '$fk' AND numero_doc LIKE '%CCF%' AND id_sucursal = $id_sucursal");
  ...
  if($total_general!=0)
  {
    ...
    $pdf->Cell(85,5,utf8_decode("NIT: ".$nite),0,1,'C');
