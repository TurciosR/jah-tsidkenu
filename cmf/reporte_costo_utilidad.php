...
//$pdf->AddPage();
$pdf->SetFont('latin','',9);
$pdf->Cell(20,5,utf8_decode("INGRESOS POR VENTA"),0,1,'L',0);
$set_x = $pdf->GetX();
$set_y = $pdf->GetY();
...
$sum_porcentaje = 0;
$sum_margen = 0;
$sql_producto = _query("SELECT SUM(fd.cantidad) as cant, FORMAT(SUM(fd.precio_venta*fd.cantidad/pre.unidad),2) as precio, SUM(pre.costo*fd.cantidad/pre.unidad) as cost, p.descripcion, p.barcode, fd.id_prod_serv as idfd, p.id_producto as idp
  FROM factura_detalle as fd, presentacion_producto as pre, producto as p, factura as f
  WHERE p.id_producto = fd.id_prod_serv AND pre.id_presentacion = fd.id_presentacion AND f.id_factura=fd.id_factura AND f.tipo_documento!='DEV' AND f.tipo_documento!='NC' AND f.anulada=0 AND f.credito=0 AND fd.fecha = '$fini' GROUP BY fd.id_prod_serv");
...
//$pdf->SetXY($set_x+20,$set_y+$linea);
$pdf->Cell(167,5,"TOTAL INGRESOS POR VENTA",0,0,'L',0);
//$pdf->SetXY($set_x+97,$set_y+$linea);
...
//$pdf->SetXY($set_x+169,$set_y+$linea);
$pdf->Cell(185,5,utf8_decode("(+) Ingresos por venta"),0,0,'R',0);
//$pdf->SetXY($set_x+187,$set_y+$linea);
