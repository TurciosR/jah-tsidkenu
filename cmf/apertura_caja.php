    ...
    if($cuenta1 == 0)
    {
        ...
        if($insertar)
        {
            ...
            if($insert_de)
            {
                $fini = date("Y-m-d");
                $dias = 10;
                $fin = sumar_dias_Ymd($fini,$dias);
                $sql = _query("SELECT COUNT(DISTINCT(id_producto)) AS cantidad FROM lote WHERE vencimiento BETWEEN '$fini' AND '$fin'");
                ...
                $inversion = number_format($row_inver["inversion"],2,".",",");
                $xdatos['prods']=$prods;
                $xdatos['inversion']=$inversion;
                $xdatos['fecha']=date("d-m-Y");
