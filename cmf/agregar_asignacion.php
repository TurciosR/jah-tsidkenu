...
function initial()
{
	...
	<?php
		...
		echo "<script src='js/funciones/funciones_asignacion.js'></script>";
	}
	else
	{
...
function insertar()
{
	...
	foreach ($array as $fila) {
		...
		$sql=_fetch_array(_query("SELECT * FROM stock_ubicacion WHERE id_sucursal=$id_sucursal AND id_ubicacion=$id_ubicacion AND id_estante=0 AND id_posicion=0 AND id_producto=$fila[id_producto] "));
