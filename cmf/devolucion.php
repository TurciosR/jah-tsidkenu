						...
						<div class="row">
							...
							<div class="col-lg-12">
								<table id="tabla" class=" table table-hover table-striped">
									...
									<?php
									$sql=_query("SELECT factura_detalle.id_factura_detalle, factura_detalle.id_prod_serv,producto.descripcion,presentacion.nombre as descripcion_pr,presentacion_producto.descripcion AS descripcion_p, factura_detalle.precio_venta,factura_detalle.cantidad,presentacion_producto.unidad,presentacion_producto.id_presentacion FROM factura_detalle JOIN producto ON producto.id_producto=factura_detalle.id_prod_serv JOIN presentacion_producto ON presentacion_producto.id_presentacion=factura_detalle.id_presentacion JOIN presentacion ON presentacion_producto.presentacion=presentacion.id_presentacion WHERE factura_detalle.id_factura=$id_factura");
...
function devolucion()
{
  ...
  $m = 1 ;
	...
	/*agregar la factura*/
	$sql_apertura = _query("SELECT * FROM apertura_caja WHERE vigente = 1 AND id_sucursal = '$id_sucursal' AND fecha='$fecha'");
	$row_apertura = _fetch_array($sql_apertura);
	$id_apertura = $row_apertura["id_apertura"];
	$turno = $row_apertura["turno"];
	$caja= $row_apertura['caja'];
