...
while ($row1=_fetch_array($sql_ip)) {
  # code...
  $sql_pr=_query("SELECT presentacion_producto.id_presentacion,presentacion.nombre,presentacion_producto.unidad FROM presentacion_producto JOIN presentacion ON presentacion_producto.presentacion=presentacion.id_presentacion WHERE id_producto=$row1[id_producto] AND presentacion_producto.id_sucursal=$_SESSION[id_sucursal] ORDER BY presentacion_producto.unidad DESC ");
  ...
  while ($row2=_fetch_array($sql_pr)) {
    ...
    $a=intdiv($existencia,$row2['unidad']);
    $existencia=$existencia-($a*$row2['unidad']);
