...
{// Page setup
  ...
  if($cuenta > 0)
  {
    ...
    while ($row_a = _fetch_array($sql_apertura))
    {
      ...
      if($fecha_actual != $fecha_ape)
      {
        ...
        $total_corteva = $total_tike + $total_factura- $monto_dev - $monto_nc  + $total_credito_fiscal  + $total_entrada_caja;
          ...
          <div class="ibox-content">
            ...
            <section>
              ...
              if($cuenta_apertura != 0 )
              {
                ...
                $sql_caja = _query("SELECT * FROM mov_caja WHERE fecha = '$fecha_apertura' AND id_apertura = '$id_apertura'   AND id_sucursal = '$id_sucursal'");
                $cuenta_caja = _num_rows($sql_caja);
                $total_entrada_caja = 0;
                $total_salida_caja = 0;
                if($cuenta_caja > 0)
                ...
                <div class="row">
                  ...
                  <table class="table table-bordered">
                    <thead>
                      ...
                      <tr>
                        <th colspan="2">Nombre: <?php echo $nombre;?></th>
                        <th>Fecha Apertura: <?php echo ED($fecha_apertura);?></th>
                        <th>Hora Apertura: <?php echo hora($hora_apertura);?></th>
                      </tr>
                      <tr>
                        <th>Monto Apertura: <?php echo "$".$monto_apertura;?></th>
                        <th>Turno: <?php echo $turno;?></th>
                        <th>Monto Registrado: <?php echo $total_corteva;?></th>
                        <th>Saldo de Caja: <?php echo $total_corte;?></th>
                      </tr>
                      <?php
                  ...
                  <tbody id="caja_x">
                    ...
                    if($cuenta_corte > 0)
                    {
                      while ($row_cc = _fetch_array($sql_cc))
                      {
                        ...
                        $total_sistema = $row_cc['totalgral'];
                        ...
                        if ($tipo_corte=="Z") {
                          ...
                          $diferencia=$total-$total_sistema;
                        }


                        if ($diferencia<0) {
                          // code...
                          $diferencia="FALTANTE: ".round(($diferencia*-1),2);
                        }
                        else {
                          if ($diferencia==0) {
                            // code...
                            ...
                            $diferencia="EXCEDENTE: ".round($diferencia,2);
                          }
                        }
                        $sql_empleadox="";
                        if ($id_empleado_c<0) {
                        ...
                        echo "<td>".round($total_sistema,2)."</td>";
  ...
  function caja()
  {
    ...
    if($cuenta > 0)
    {
      ...
      echo "<input type='hidden' id='aper_id' name='aper_id' value='".$id_apertura."'>";
      /////////////////////////////////////////////////////////////////////////////////////////////
      $sql_corte = _query("SELECT * FROM factura WHERE fecha = '$fecha_apertura' AND id_apertura = '$id_apertura'   AND id_sucursal = '$id_sucursal' AND tipo_documento!='DEV' AND finalizada = 1 AND anulada = 0 AND credito = 0");
      ...
      if($cuenta_caja > 0)
      {
        while ($row_caja = _fetch_array($sql_caja))
        {
          ...
          if($entrada == 1)
          {
            $total_entrada_caja = $total_entrada_caja+ $monto;
          }
          else if($salida == 1)
          {
            $total_salida_caja = $total_salida_caja+ $monto;
          }
        }
      }

      $sql_monto_dev=_fetch_array(_query("SELECT SUM(factura.total) AS total_devoluciones FROM factura  WHERE factura.tipo_documento ='DEV' AND factura.id_apertura_pagada=$id_apertura"));
      $monto_dev=$sql_monto_dev['total_devoluciones'];

      echo "$monto_dev";

      $sql_monto_dev=_fetch_array(_query("SELECT SUM(factura.total) AS total_devoluciones FROM factura WHERE factura.tipo_documento ='NC' AND factura.id_apertura_pagada=$id_apertura"));
      $monto_nc=$sql_monto_dev['total_devoluciones'];

      $total_corte = $total_tike + $total_factura- $monto_dev - $monto_nc  + $total_credito_fiscal + $monto_apertura  + $total_entrada_caja - $total_salida_caja;
      $total_corteva = $total_tike + $total_factura- $monto_dev - $monto_nc  + $total_credito_fiscal ;
    ...
    function search()
    {
      ...
      if($cuenta_corte > 0)
      {
        while ($row_cc = _fetch_array($sql_cc))
        {
          ...
          $id_apertura = $row_cc["id_apertura"];
          $tipo_corte = $row_cc["tipo_corte"];
          $total = $row_cc["cashfinal"];/*manual*/

          $total_sistema = $row_cc["totalgral"];/*sistema*/
          $diferencia = $row_cc["diferencia"];
          $turno = $row_cc["turno"];
          ...
          $lista.= "<td>".round($total,2)."</td>";
          $lista.= "<td>".round($total_sistema,2)."</td>";
