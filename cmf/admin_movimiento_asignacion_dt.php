...
$columns = array(
  ...
  'formatter' => function( $id_movimiento, $row ){
    ...
    $admin=$_SESSION["admin"];
        ...
        if ($tipo=="TI") {
          $filename='reporte_transferencia_detalle.php';
          $link=permission_usr($id_user,$filename);
          if ($link!='NOT' || $admin=='1' ){
            $menudrop.= "<li><a  href='reporte_transferencia_detalle.php?id_movimiento=".$id_movimiento."' target='_blank'><i class=\"fa fa-file-pdf-o\"></i> Imprimir PDF</a></li>";
            }
        }

        if ($tipo=="TI") {
            $menudrop.= "<li><a data-toggle='modal' href='imprimir_transferencia.php?id_movimiento=".$id_movimiento."' data-target='#viewModal' data-refresh='true'><i class=\"fa fa-print\"></i> Imprimir TIK</a></li>";
        }
