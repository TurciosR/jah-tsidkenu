	  ...
	  $hora_pos = date_format($hora_pre, 'g:i A');
		...
		if($cuenta > 0)
		{
			...
			while ($row = _fetch_array($sql_lista))
			{
				...
				$total = number_format($row["valor"],2,".",",");
				$lca = strlen($total);
				$esp =str_repeat(" ",(16 - $lca));
				$lcn = strlen($numero_doc);
				$espn =str_repeat(" ",(12 - $lcn));
				$lch = strlen($hora);
				$esph =str_repeat(" ",(8 - $lch));
				$info_mov.= $numero_doc.$espn.$hora.$esph.$esp."$".$total."\n";
			}
			$info_mov .= "---------------------------------------\n";
			$lct = strlen($tot);
			$espt =str_repeat(" ",(29 - $lct));
			$info_mov.= str_pad("TOTAL VALES",20," ",STR_PAD_RIGHT).str_pad("$".number_format($tot,2,".",","),18," ",STR_PAD_LEFT)." \n\n\n";
		...
		if($cuenta > 0)
		{
			...
			$lct = strlen($tot);
			$espt =str_repeat(" ",(29 - $lct));
			$info_mov.= str_pad("TOTAL DEV",20," ",STR_PAD_RIGHT).str_pad("$".number_format($tot,2,".",","),18," ",STR_PAD_LEFT)." \n\n\n";
