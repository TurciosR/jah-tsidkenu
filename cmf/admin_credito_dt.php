...
$id_sucursal=$_SESSION['id_sucursal'];
$joinQuery ="  FROM credito LEFT JOIN cliente ON cliente.id_cliente=credito.id_cliente JOIN factura ON factura.id_factura=credito.id_factura";
...
$columns = array(
  ...
  array( 'db' => 'CONCAT(`factura`.`nombre`," ",factura.num_fact_impresa)','as' => "nom", 'dt' => 3, 'field' => 'nom'),
  array( 'db' => '`credito`.`tipo_doc`', 'dt' =>4, 'field' => 'tipo_doc' ),
  array( 'db' => '`credito`.`numero_doc`', 'dt' =>5, 'field' => 'numero_doc' ),
  array( 'db' => '`credito`.`total`', 'dt' =>6, 'field' => 'total' ),
  array( 'db' => '`credito`.`abono`', 'dt' =>7, 'field' => 'abono' ),
  array( 'db' => '`credito`.`saldo`', 'dt' =>8, 'field' => 'saldo' ),
