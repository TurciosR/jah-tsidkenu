...
$(document).on('change', '#destino', function(event) {
  ...
  $("#estante").val("");
  $("#estante").empty().trigger('change');
  $.ajax({
    type: "POST",
    url: "ingreso_inventario.php",
    data: "process=val&id_ubicacion=" + id_ubicacion,
    dataType: "JSON",
    success: function(datax) {
      if (datax.typeinfo == "Success") {
        $("#estante").html(datax.opt);
        $("#estante").trigger('change');
      }
    }
  });
});
