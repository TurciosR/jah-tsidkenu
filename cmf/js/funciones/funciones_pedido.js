$(document).ready(function() {
	...
	//check de Reservado
	$("#checkreser").on("ifChecked", function(event) {
      $("#reservado").val("1");
    });
    $("#checkreser").on("ifUnchecked", function(event) {
      $("#reservado").val("0");
    });
});
...
function agregar_producto(id_prod, descrip) {
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			tr_add += "<td><div class='col-xs-1'><input type='text' value='0'  class='form-control cant "+data.categoria+"' style='width:60px;'></div></td>";
			...
			function agregar_producto_proceso(id_prod, descrip) {
				...
				$.ajax({
					...
					success: function(data)
					{
						...
						tr_add += "<td><div class='col-xs-1'><input type='text' readonly class='form-control' value='0'></div></td>";
						tr_add += "<td><div class='col-xs-1'><input type='text' value='0'  class='form-control cant "+data.categoria+"' style='width:60px;'></div></td>";
		...
		function senddata()
		{
			...
			display_notify('Warning', 'Verifique lugar de entrega!');
		...
		function reload1()
		{
			location.href = "admin_pedidos.php";
		}
		function anular() {
			var id_pedido = $('#id_pedido').val();
		...
		function generar1()
		{
			...
			dataTable = $('#editable2').DataTable({
				...
				"serverSide": true,
				"ajax":{
					url :"admin_pedido_dt.php?fini="+fini+"&fin="+fin,
					error: function()
					{  // error handling
