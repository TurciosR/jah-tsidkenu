  ...
  $.ajax({
    ...
    success: function(data)
    {
      ...
      for(var i=0;i<iphp;i++)
      {
        ...
        tr_add += "<td><div class=''><input type='text'  class='form-control cant' style='width:80px;'></div></td>";
        tr_add += "<td class=''>" + caduca + '</td>';
        tr_add += "<td class=''>" + posiciones + '</td>';
        tr_add += "<td class='Delete text-center'><button class='btn btn-danger'><i class='fa fa-trash'></i></button></td>";
        tr_add += '</tr>';
      }
      if(i!=0)
      {
...
function senddata()
{
  ...
  $("#inventable>tbody tr").each(function()
  {
    ...
    var vence = $(this).find(".vence").val();
    if (compra!="" &&parseFloat(compra) >= 0 &&venta!="" &&parseFloat(venta) > 0 && cant != "" && parseInt(cant)>0)
    {
