...
function generar() {
  ...
  dataTable = $('#editable2').DataTable({
    ...
    "serverSide": true,
    "ajax": {
      url: "admin_movimiento_asignacion_dt.php",
      "data": {
        "origen": origen,
        "pro":pro,
      },
...
$(document).on('click', '.imprimir_transferencia', function(event) {
  imprimir_mov($(this).attr("id_movimiento"));
});

function imprimir_mov(id_movimiento){
	var datoss = "process=imprimir_transferencia"+"&id_movimiento="+id_movimiento;
	$.ajax({
		type : "POST",
		url :"imprimir_traslado.php",
		data : datoss,
		dataType : 'json',
