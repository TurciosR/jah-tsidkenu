  ...
  $.ajax({
    ...
    success: function(datax) {
      ...
      if (tipo == 'Z' || tipo == 'X') {
        ...
        fila1 += "<td style='text-align: center'><label id='id_total_general'>" + round(total_corte1, 2) + "</label></td>";
        fila1 += "<td style='text-align: center'><label id='id_diferencia'>" + round(total_corte1, 2) + "</label></td></tr>";
      }
...
$(document).on("keyup", "#total_efectivo", function() {
  ...
  var valor = parseFloat(total_efectivo - total_corte);
...
function corte() {
  ...
  $.ajax({
    ...
    success: function(datax) {
      ...
      if (datax.typeinfo == "Success")
      {
        var total = parseFloat($("#id_total").text().replace(",","")).toFixed(2);
        var liquido = parseFloat($("#id_total_general").text().replace(",","")).toFixed(2);
        var devoluciones = parseFloat($("#id_total_dev").text().replace(",","")).toFixed(2);
        var entradas = parseFloat($("#tentr").text().replace(",","")).toFixed(2);
        var salidas = parseFloat($("#tsalt").text().replace(",","")).toFixed(2);
        var diferencia = parseFloat($("#id_diferencia").text().replace(",","")).toFixed(2);
        var efectivo = parseFloat($("#total_efectivo").val().replace(",","")).toFixed(2);
        ...
        msg = "FARMACIA LA FE 1: CORTE DE CAJA "+hora+"; TOTAL:"+total+", DEVOLUCIONES:"+devoluciones;
        msg += " ENTRADAS:"+entradas+", SALIDAS:"+salidas+", LIQUIDO:"+liquido+", EN CAJA:"+efectivo+", DIFERENCIA:"+diferencia;
