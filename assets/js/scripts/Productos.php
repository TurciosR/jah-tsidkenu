	...
	*/
	...
	function get_data(){
		...
		if ($row != 0) {
			...
			foreach ($row as $rows) {
				...
				$data[] = array(
					$rows->codigo_generico." | ".$rows->codigo_barra,
					$rows->categoria,
					$rows->marca,
					$rows->modelo,
					$show_text,
	...
	function agregar(){
		if($this->input->method(TRUE) == "GET"){
			...
			$this->utils->begin();
     $row_existe = $this->utils->get_one_row("producto", array('modelo'=> $modelo,));
		 if($row_existe==NULL){
			...
			if($id_producto!=NULL){
				...
				$xdatos["type"]="error";
				$xdatos['title']='Alerta';
				$xdatos["msg"]="Ya  existe un registro con esa modelo!!!";
			}
