...
function new_producto(id_producto, nombre, id_color, qty = 1) {
  ...
  if ($("#proceso").val() == "editar") {
    id_trabajo_taller = $("#id_trabajo_taller").val();
  }
  ...
  if (id_color == "SERVICIO") {
    ...
    $.ajax({
      ...
      data: "id=" + id_producto + "&csrf_test_name=" + token + "&id_s=" + id_color + "&id_trabajo_taller=" + id_trabajo_taller,
      dataType: 'json',
      success: function (datax) {
        ...
        fila += "<td style='width:10%;'><input type='hidden' class='form-control costo decimal' value='" + datax.costo + "' style='width:100%;'><input type='hidden' class='form-control precio_minimo decimal' value='" + datax.precio_minimo + "' style='width:100%;'>-</td>";
  ...
  if (distinto && id_color != "SERVICIO") {
    $.ajax({
      ...
      data: "id=" + id_producto + "&csrf_test_name=" + token + "&id_s=" + id_color + "&id_trabajo_taller=" + id_trabajo_taller + "&clasifica=" + porc_clasifica,
      dataType: 'json',
      success: function (datax) {
...
function fin_data() {
  var id_trabajo_taller = $("#id_trabajo_taller").val();
  var errors = false;
  var error_array = [];
  var array_json = [];
  var cuantos = 0;
  ...
  if (errors == false) {
    ...
    var urls = url + "/get_data_cliente/" + id_trabajo_taller;
    $.ajax({
      type: 'POST',
      url: url + '/finalizar',
      cache: false,
...
function save_data_cte() {
  ...
  var id_factura = $("#id_tr_taller").val();
...
function carga_venta() {
  ...
  $.ajax({
    ...
    success: function (data) {
      notification(data.type, data.title, data.msg);
      if (data.type == "success") {
        $("#id_trabajo_taller").val(data.id_trabajo_taller);
...
function llenar_prods(arr_prod) {
  $.each(arr_prod, function (i, item) {
    nombre = item.nombre + " " + item.marca + " " + item.modelo + " " + item.color;
    let fila = "<tr>";
...
function fin_data_ref() {
  ...
  if (errors == false) {
    ...
    if (parseInt(id_trabajo_taller) < 0 || id_trabajo_taller == "") {
      id_trabajo_taller = -1;
    }
...
function save_new_cte() {
  ...
  if (isValid == true) {
    $.ajax({
      ...
      success: function (data) {
        notification(data.type, data.title, data.msg);
        notification('info', 'Alerta', 'Por favor espere a que el cliente sea seleccionado.');
        if (data.type == "success") {
