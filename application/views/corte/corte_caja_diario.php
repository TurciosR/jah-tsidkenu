              ...
              <form id="form_corte_caja" novalidate>
                ...
                <div class="row">
                  ...
                  <div class="col-lg-6">
                    <div class="form-group single-line">
                      <div class='alert alert-success text-center' style='font-weight: bold;'>
                        <label style='font-size: 15px;'>Total Documentos (pago con tarjeta)</label>
                      </div>
                      ...
                      <table class="table table-border">
                        ...
                        <tbody id='tabla_doc'>
                          <tr>
                            <td>TIQUETE</td>
                            <td><input type="hidden" id="t_min" name="t_min" value="<?= $ticket_rowmm_tarjeta->minimo ?>"><?php if ($ticket_rowmm_tarjeta !== NULL) {
                                                                                                                              echo   $ticket_rowmm_tarjeta->minimo;
                                                                                                                            } ?></td>
                            <td><input type="hidden" id="t_max" name="t_max" value="<?= $ticket_rowmm_tarjeta->maximo ?>"><?php if ($ticket_rowmm_tarjeta !== NULL) {
                                                                                                                              echo  $ticket_rowmm_tarjeta->maximo;
                                                                                                                            } ?></td>
                            <td><input type="hidden" id="t_count" name="t_count" value="<?= $count_rango_tarjeta_tik ?>"><?= $count_rango_tarjeta_tik; ?></td>
                            <td><input type="hidden" id="t_total" name="t_total" value="<?= $total_tarjeta_tik ?>"><?php echo number_format($total_tarjeta_tik, 2, ".", ","); ?></td>
                            <!--
                      ...
                      -->
                          ...
                          <tr>
                            <!--agregar los hidden para las 5 filas de esto porque van en la tabla controlcaja en un solo cmapo igual que el resto de cortes y validar cada uno C, X, Z-->
                            <td>FACTURA</td>
                            <td><input type="hidden" id="cof_min" name="cof_min" value="<?= $cof_rowmm_tarjeta->minimo ?>"><?php if ($cof_rowmm_tarjeta !== NULL) {
                                                                                                                              echo $cof_rowmm_tarjeta->minimo;
                                                                                                                            } ?></td>
                            <td><input type="hidden" id="cof_max" name="cof_max" value="<?= $cof_rowmm_tarjeta->maximo ?>"><?php if ($cof_rowmm_tarjeta !== NULL) {
                                                                                                                              echo $cof_rowmm_tarjeta->maximo;
                                                                                                                            } ?></td>
                            <td><input type="hidden" id="cof_count" name="cof_count" value="<?= $count_rango_tarjeta_cof ?>"><?= $count_rango_tarjeta_cof; ?></td>
                            <td><input type="hidden" id="cof_total" name="cof_total" value="<?= $total_tarjeta_cof ?>"><?php echo number_format($total_tarjeta_cof, 2, ".", ","); ?></td>

                          </tr>
                          <tr>
                            <td>CREDITO FISCAL</td>
                            <td><input type="hidden" id="ccf_min" name="ccf_min" value="<?= $ccf_rowmm_tarjeta->minimo ?>"><?php if ($ccf_rowmm_tarjeta !== NULL) {
                                                                                                                              echo $ccf_rowmm_tarjeta->minimo;
                                                                                                                            } ?></td>
                            <td><input type="hidden" id="ccf_max" name="ccf_max" value="<?= $ccf_rowmm_tarjeta->maximo ?>"><?php if ($ccf_rowmm_tarjeta !== NULL) {
                                                                                                                              echo $ccf_rowmm_tarjeta->maximo;
                                                                                                                            } ?></td>
                            <td><input type="hidden" id="ccf_count" name="ccf_count" value="<?= $count_rango_tarjeta_ccf ?>"><?= $count_rango_tarjeta_ccf; ?></td>
                            <td><input type="hidden" id="ccf_total" name="ccf_total" value="<?= $total_tarjeta_ccf ?>"><?php echo number_format($total_tarjeta_ccf, 2, ".", ","); ?></td>
                          </tr>
                          <tr>
                            <td>TOTAL</td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td><input type="hidden" id="total_tarjeta_fin" name="total_tarjeta_fin" value="<?= $total_tarjeta_fin; ?>"><label id="id_total"><?php echo number_format($total_tarjeta_fin, 2, ".", ","); ?></label></td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                <div class="row">
                  <table class="table table-border" id="table_t">
                    <thead>
                      <tr>
                        <!-- aniadir un campo que muestre el dinero en efectivo y el dinero en tarjeta y luego guardar la suma en total_efectivo -->
                    ...
                    <tbody id="table_data">
                      <tr>
                        <td>
                          ...
                          <!-- TODO: Agregar el total de dinero en tarjetas tambien al corte de caja sin afectar la funcionalidad -->
                          <!-- SE ANIADIO EL TOTAL DE PAGO CON TARJETA, QUE TAMBIEN DEBE SER INCLUIDO EN EL CORTE DE CAJA -->
                          <!-- MUY PROBABLEMENTE, EN EL NEGOCIO SE HACIA UNA VENTA CON TARJETA, SE DEVOLVIA -->
                          <!-- Y LUEGO SE HACIA EL PAGO EN EFECTIVO PARA QUE ESTE FUERA ANIADIDO A LA CANTIDAD EN EL CORTE DE CAJA -->
