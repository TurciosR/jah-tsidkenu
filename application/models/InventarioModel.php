...
class InventarioModel extends CI_Model
{
	...
	function get_collection_carga($order, $search, $valid_columns, $length, $start, $dir,$id_sucursal)
	{
		if ($order !=	 null) {
		...
		$this->db->select("cp.requiere_imei,cp.imei_ingresado,cp.id_carga,DATE_FORMAT(cp.fecha,'%d-%m-%Y') as fecha, TIME_FORMAT(hora,'%h:%i %p') as hora, FORMAT(cp.total,2) as total, cp.concepto, cp.correlativo, u.nombre");
		$this->db->limit($length, $start);
		$this->db->from('inventario_carga as cp');
	...
	function total_rows_carga(){
		$clients = $this->db->get("inventario_carga");
		if ($clients->num_rows() > 0) {
...
function get_productos($query){
	$this->db->select('id_producto, nombre, marca, modelo, codigo_barra');
	$this->db->where("(modelo LIKE '%".$query."%' OR nombre LIKE '%".$query."%' OR marca LIKE '%".$query."%' OR codigo_barra LIKE '%".$query."%')", NULL, FALSE);
	$this->db->where('activo', 1);
...
function get_detail_ci($id_carga){
	...
	$this->db->select("producto_color.color,icd.*, p.nombre, p.modelo, p.marca, p.codigo_barra, p.imei,p.n_imei");
...
/***************************************************************************************/
/***************************************************************************************/
function get_collection_descarga($order, $search, $valid_columns, $length, $start, $dir, $id_sucursal)
{
	if ($order !=	 null) {
		$this->db->order_by($order, $dir);
	}
	...
	$this->db->select("cp.requiere_imei,cp.imei_ingresado,cp.id_descarga,DATE_FORMAT(cp.fecha,'%d-%m-%Y') as fecha, TIME_FORMAT(hora,'%h:%i %p') as hora, FORMAT(cp.total,2) as total, cp.concepto, cp.correlativo, u.nombre");
	$this->db->limit($length, $start);
	$this->db->from('inventario_descarga as cp');
	$this->db->join('usuario as u', 'cp.id_usuario = u.id_usuario');
	$this->db->where("cp.id_sucursal",$id_sucursal);
	$clients = $this->db->get();
	if ($clients->num_rows() > 0) {
...
function total_rows_descarga(){
	$clients = $this->db->get("inventario_descarga");
	if ($clients->num_rows() > 0) {
		return $clients->num_rows();
	} else {
		return 0;
...
function get_detail_di($id_carga){
	...
	$this->db->select("producto_color.color,icd.*, p.nombre, p.modelo,p.imei,p.n_imei");
...
function get_collection_ajuste($order, $search, $valid_columns, $length, $start, $dir)
{
	...
	$this->db->select("cp.requiere_imei,cp.imei_ingresado,cp.id_ajuste,DATE_FORMAT(cp.fecha,'%d-%m-%Y') as fecha, TIME_FORMAT(hora,'%h:%i %p') as hora, FORMAT(cp.total,2) as total, cp.concepto, cp.correlativo, u.nombre");
	$this->db->limit($length, $start);
...
function get_detail_aj($id_carga){
	...
	$this->db->select("producto_color.color,icd.*, p.nombre,p.imei,p.n_imei,p.modelo");
